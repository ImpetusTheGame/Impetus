...
///
/// 09/13/19 || msg: "New Editor script, Fixed Stamina, Fixed Crouching, Put 'FOV Kick' Under reconstruction, made dynamic foot steps easier to understand." || Author Signature: Aedan Graves || version: 19.7.28cu 
...
public class FirstPersonAIO : MonoBehaviour {
    ...
    public  float fOVToMouseSensitivity,FOVToMouseInternal = 1;
...
public class BETA_SETTINGS{
    ...
    private void Update()
    {
        #region Look Settings - Update
            ...
            targetAngles.y += mouseYInput * (mouseSensitivityInternal - ((baseCamFOV-camFOV)*FOVToMouseInternal)/6f);
            targetAngles.x += mouseXInput * (mouseSensitivityInternal - ((baseCamFOV-camFOV)*FOVToMouseInternal)/6f);
    ...
    if(advanced.maxSlopeAngle>0){
        ...
        fps_Rigidbody.AddForce(Physics.gravity * (advanced.gravityMultiplier * 5));
...
/*     public IEnumerator FOVKickOut()
    ...
    public class FPAIO_Editor : Editor{
        ...
        public override void OnInspectorGUI(){
            ...
            t.enableCameraShake = EditorGUILayout.ToggleLeft(new GUIContent("Enable Camera Shake?", "Call this Coroutine externaly with duration ranging from 0.01 to 1, and a magnitude of 0.01 to 0.5."), t.enableCameraShake);
            ...
            EditorGUILayout.Space();
            EditorGUILayout.Space();
            t.playerCanMove = EditorGUILayout.ToggleLeft(new GUIContent("Can Player Move?","Determines if the player is allowed to move."),t.playerCanMove);
            ...
            if(showCrouchMods){
                ...
                t._crouchModifiers.crouchWalkSpeedMultiplier = EditorGUILayout.Slider(new GUIContent("Crouch Movement Speed","Determines how fast the player can move while crouching."),t._crouchModifiers.crouchWalkSpeedMultiplier,0.01f,1.5f);
            ...
            if(showFOVKickSet){
                GUILayout.Label("Under Construction",new GUIStyle(GUI.skin.label){alignment = TextAnchor.MiddleCenter,fontStyle = FontStyle.Bold, fontSize = 13},GUILayout.ExpandWidth(true));
                GUI.enabled = false;
            ...
            if(showAdvanced){
                ...
                t.advanced.zeroFrictionMaterial = (PhysicMaterial)EditorGUILayout.ObjectField(new GUIContent("Zero Friction Material","A Physic material with a friction value of 0. This is needed for smooth player movement."),t.advanced.zeroFrictionMaterial,typeof(PhysicMaterial),false);
                t.advanced.highFrictionMaterial = (PhysicMaterial)EditorGUILayout.ObjectField(new GUIContent("High Friction Material","A Physic material with a max friction value. This is needed for smooth player movement."),t.advanced.highFrictionMaterial,typeof(PhysicMaterial),false);
